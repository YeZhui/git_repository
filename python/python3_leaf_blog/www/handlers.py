...
def api_get_blog_by_classes_id(request, *, classes_id, page='1'):
	...
	if num == 0:
		...
		blogs = yield from Blog.findAll(orderBy = 'created_at desc', where="classes_id='%s'" % classes_id, limit = (p.offset, p.limit))
		date_blogs = yield from Blog.findAll(orderBy = 'created_at desc')
	num = yield from Classification.findNumber('count(id)')
	if num == 0:
		classes = []
...
def api_get_blog_by_date(request, *, blog_date, page='1'):
	...
	if num == 0:
		...
		blogs = yield from Blog.findAll(orderBy = 'created_at desc', where="blog_date='%s'" % blog_date, limit = (p.offset, p.limit))
		date_blogs = yield from Blog.findAll(orderBy = 'created_at desc')
	num = yield from Classification.findNumber('count(id)')
	if num == 0:
		classes = []
...
def upload_userimg(request, *, FILES):
	...
	content = file.read()
	imgPath = '/static/img/' + request.__user__.id + '_USER' + '.' + filename
	path = '.' + imgPath
...
#写文章时上传图片
@post('/upload/img')
def upload_img(request, *, guid):
	files = request.editormdImage['editormd-image-file']
	filename = files.filename
	file = files.file
	content = file.read()
	imgPath = '/static/img/' + request.__user__.id 
	path = '.' + imgPath
	if not os.path.exists(path):
		os.mkdir(path)
	t = time.time()
	path = path + '/' + str(int(round(t*1000))) + '_' + filename
	with open(path, 'wb+') as dest:
		dest.write(content)
